    ...
    setTimeout(async () => {
      ...
      try {
        const stages = await win.webContents.executeJavaScript(
          ...
          'function txt(){var t=document.querySelector(".dshwv-text");return t?String(t.textContent||"").replace(/\s+/g," ").slice(0,34):""}' +
          'return new Promise(function(res){' +
          '  setTimeout(function(){' +
          '    out.push("初始|"+txt());var i=0;' +
          '    function step(){if(i>=4){res(JSON.stringify(out));return}bubble.click();i++;' +
          '      setTimeout(function(){out.push("第"+i+"次|"+txt());step()},600)}' +
          '    step()},1400)})' +
          '}catch(e){return JSON.stringify({err:String(e.message)})}})()'
        )
