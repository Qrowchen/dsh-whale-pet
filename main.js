...
async function getBalance() {
  ...
  balanceInFlight = (async () => {
    ...
    if (payload.ok) {
      ...
      // 本次启动后的第一次记账打 'start' 标记，用于回看"什么时候开机、当时多少钱"
      const led = recordLedger(Number(payload.totalBalance), ledgerBootPending ? 'start' : null)
      ledgerBootPending = false
...
// 单日流水条数上限。按 60 秒查一次算，400 条约 6.6 小时；
// 只影响"能回看多久的明细"，不影响累计值本身。
const LEDGER_LOG_MAX = 400
// 启动后首次记账要打上 'start' 标记，用来回答"什么时候开机、当时多少钱"
let ledgerBootPending = true

// 日期键必须用【本地日期】。
// 曾经用 new Date().toISOString().slice(0,10) —— 那是 UTC 日期，
// 东八区在凌晨 0:00-7:59 时 UTC 还停在前一天，一到早上 8:00 就会跳变，
// 于是触发下面的"跨天重置"，把当天凌晨已经累计的消耗清零。
...
//
// 为什么不能直接 JSON.parse：
//   读取失败时若静默返回默认值，等于把当天已累计的消耗清零 —— 而这正是
//   「重启后今日消耗变 0」最隐蔽的一种成因。实测过两种情况：
//     1) 文件带 UTF-8 BOM（PowerShell 的 Set-Content -Encoding UTF8 会写 BOM）
//     2) 写入中途被打断导致 JSON 截断
//   所以这里：剥掉 BOM；解析失败时先把损坏内容另存为 .broken 再返回 null，
//   至少不会连原始数据都丢掉。
function readLedgerSafe() {
  let raw = null
  try { raw = fs.readFileSync(LEDGER_PATH, 'utf8') } catch (err) { return null }
  if (raw.charCodeAt(0) === 0xFEFF) raw = raw.slice(1)
  try {
    const o = JSON.parse(raw)
    return (o && typeof o === 'object') ? o : null
  } catch (err) {
    try { fs.writeFileSync(LEDGER_PATH + '.broken', raw, 'utf8') } catch (e) {}
    console.error('[记账] 账本解析失败，已备份为 usage-ledger.json.broken：' + err.message)
    return null
  }
}
...
// 记一条观测流水，并返回当前账本。
//
// 与"只存一个 lastBalance"的旧实现相比，这里额外保留
// 【时间 + 余额 + 本次差额】的流水（led.log），因此：
//   - 重启 / 掉线后能接着上次的余额继续算，不会因为中途退出就断掉
//   - 可以回看什么时候开机、当时余额多少、什么时候掉的
//   - 跨天时把昨天汇总进 history，保留最近 30 天
//
// event 用于标注特殊节点，目前只有 'start'（本次开机后的首次观测）。
function recordLedger(balance, event) {
  const today = localDateKey()
  const now = new Date().toISOString()

  let led = { date: today, todayUsage: 0, lastBalance: null, log: [], history: [] }
  const prev = readLedgerSafe()
  if (prev) led = Object.assign(led, prev)
  if (!Array.isArray(led.log)) led.log = []
  if (!Array.isArray(led.history)) led.history = []

  if (led.date !== today) {
    // 跨天：先把昨天汇总进 history，再重置当天。
    // 注意 lastBalance 要重新建立基准（置 null），否则会把昨天的余额差算进今天。
    if (led.date) {
      led.history.unshift({
        date: led.date,
        usage: Number(led.todayUsage) || 0,
        endBalance: led.lastBalance,
        archivedAt: now,
      })
      led.history = led.history.slice(0, 30)
    }
    led = { date: today, todayUsage: 0, lastBalance: null, log: [], history: led.history }
  }

  let delta = null
  if (isFinite(balance)) {
    if (led.lastBalance !== null && balance < led.lastBalance) {
      delta = Number((led.lastBalance - balance).toFixed(6))
      led.todayUsage = Number(((led.todayUsage || 0) + delta).toFixed(6))
    }
  ...
  led.log.push({
    t: now,
    b: isFinite(balance) ? balance : null,
    d: delta,            // 本次观测到的下降额；null 表示无变化或首次建立基准
    e: event || null,    // 'start' = 开机后首次观测
  })
  if (led.log.length > LEDGER_LOG_MAX) led.log = led.log.slice(-LEDGER_LOG_MAX)
    ...
    setTimeout(async () => {
      ...
      try {
        const stages = await win.webContents.executeJavaScript(
          ...
          'var img=document.querySelector(".dshwv-img");if(!img)return JSON.stringify({err:"no img"});' +
          'var r=img.getBoundingClientRect();' +
          'if(window.__whaleDiag&&window.__whaleDiag.showBubble)window.__whaleDiag.showBubble();' +
          'document.dispatchEvent(ev("pointerdown",r.left+r.width*0.5,r.top+r.height*0.6,1));' +
          'document.dispatchEvent(ev("pointerup",r.left+r.width*0.5,r.top+r.height*0.6,0));' +
          'var out=[];var bubble=document.querySelector(".dshwv-bubble");' +
          'function txt(){var t=document.querySelector(".dshwv-text");return t?String(t.textContent||"").replace(/\s+/g," ").slice(0,30):""}' +
          'function exprName(){var im=document.querySelector(".dshwv-img");var s=im?String(im.src||""):"";var m=s.match(/name=([a-z_0-9]+)/);if(m)return m[1];if(s.indexOf("image.png")>=0)return "main";return "?"}' +
          'return new Promise(function(res){' +
          '  setTimeout(function(){' +
          '    out.push("初始|"+txt()+" 表情="+exprName());var i=0;' +
          '    function step(){if(i>=4){res(JSON.stringify(out));return}bubble.click();i++;' +
          '      setTimeout(function(){out.push("第"+i+"次|"+txt()+" 表情="+exprName());step()},600)}' +
          '    step()},1400)})' +
          '}catch(e){return JSON.stringify({err:String(e.message)})}})()'
        )
        const arr = JSON.parse(stages)
        if (Array.isArray(arr)) { for (const s of arr) console.log('[诊断] 三态 ' + s) }
        else console.log('[诊断] 三态顺序: ' + stages)
      } catch (err) {
        console.error('[诊断] 三态验证失败: ' + String(err && err.message))
      }
